...
class BaseRecurrent(nn.Module):
    def __init__(self, input_size, h_size, dropout):
        ...
        self.hidden_size = h_size
        self.recurrent = nn.RNN(input_size=input_size, hidden_size=self.hidden_size, dropout=dropout)
        self.final = nn.Linear(in_features=self.hidden_size,out_features=1)
        self.dropout = nn.Dropout(p=dropout)
        self.sigmoid = nn.Sigmoid()
...
class BaseLSTM(nn.Module):
    def __init__(self, input_size, h_size, dropout):
        ...
        self.hidden_size = h_size
        self.lstm = nn.LSTM(input_size=input_size, hidden_size=self.hidden_size, dropout=dropout)
        self.final = nn.Linear(in_features=self.hidden_size, out_features=1)
        self.dropout = nn.Dropout(p=dropout)
        self.sigmoid = nn.Sigmoid()
