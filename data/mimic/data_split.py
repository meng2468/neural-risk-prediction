...
data_x = pd.read_csv(data_dir+'mimic_prepared_x.csv',index_col='stay_id')
...
def reset_ids(df):
    ...
    df = df.set_index('id')
    return df.drop(columns='stay_id')

reset_ids(data_x.loc[train]).to_csv(data_dir+'mimic_train_x.csv')
reset_ids(data_x.loc[test]).to_csv(data_dir+'mimic_test_x.csv')
reset_ids(data_x.loc[val]).to_csv(data_dir+'mimic_val_x.csv')
